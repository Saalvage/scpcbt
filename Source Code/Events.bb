			...
			Case "alarm"
				...
				If e\EventState = 0.0 Then
					If PlayerRoom = e\room Then
						...
						If e\room\NPC[0] = Null Then
							...
							SetNPCFrame(e\room\NPC[3], 286) : e\room\NPC[3]\State = 8.0
				...
				Else
					...
					If e\EventState >= 500.0 Then
						...
						If e\EventState2 = 0.0 Then
							...
							If e\EventState > 900.0 And e\room\RoomDoors[5]\Open Then
								...
								Else
									...
									If e\room\NPC[2]\State <> 1.0 And KillTimer >= 0.0 Then
										If EntityZ(e\room\NPC[2]\Collider) < e\room\z - 1150.0 * RoomScale Then
											...
											BlinkTimer = -10.0
											PlaySound2(StoneDragSFX, Camera, Curr173\Collider)
											If EntityDistanceSquared(Curr173\Collider, Collider) < PowTwo(2.5) And Abs(EntityY(Collider) - EntityY(Curr173\Collider)) < 1.0 Then
						...
						If EntityDistanceSquared(e\room\Objects[0], Collider) < PowTwo(2.5) Then
					...
					If ((e\EventState Mod 600 > 300) And ((e\EventState + FPSfactor) Mod 600 < 300)) Then
						i = Floor((e\EventState - 5000.0) / 600.0) + 1
			...
			Case "173" ;the intro sequence
				...
				If KillTimer >= 0.0 And e\EventState2 = 0.0 Then
					...
					If e\EventState3 > 0.0 Then
						...
						If e\EventState3 < 170.0 Then
							...
							If e\EventState3 < 15.0 Then
								...
								Else
									...
									DropSpeed = 0.0
									e\EventState3 = 15.0
									Msg = GetLocalString("Messages", "pickpaper")
									MsgTimer = 70.0 * 7.0
								EndIf
						...
						ElseIf e\EventState3 >= 150.0 And e\EventState3 < 700.0
							...
							If e\room\NPC[3]\State = 7.0 Then
								...
								If ChannelPlaying(e\room\NPC[3]\SoundCHN2) = False
							...
							Else
								...
								;Outside the cell
								If DistanceSquared(EntityX(Collider), e\room\x - (3072.0 + 1024.0) * RoomScale, EntityZ(Collider), e\room\z + 192.0 * RoomScale) > PowTwo(1.5) Then
								...
								Else ;Inside the cell
									...
									ElseIf e\EventState3 > 630.0
										...
										If e\room\RoomDoors[5]\Open = True Then 
											...
											em\RandAngle = 7 : em\Speed = 0.03 : em\SizeChange = 0.003 : em\Room = PlayerRoom
						...
						ElseIf e\EventState3 < 800.0
							...
							If e\room\NPC[5]\State <> 11.0 Then
								If EntityDistanceSquared(e\room\NPC[3]\Collider, e\room\NPC[5]\Collider) > PowTwo(5) And EntityDistance(e\room\NPC[4]\Collider, e\room\NPC[5]\Collider) Then ;TODO: check what this is
									If EntityDistanceSquared(e\room\NPC[5]\Collider, Collider) < PowTwo(3.5) Then
										For i = 3 To 5
											If i < 5 Then
						...
						ElseIf e\EventState3 < 900.0
							...
							If e\room\NPC[6] <> Null Then
								If e\room\NPC[6]\State = 0.0 Then 
									If e\room\RoomDoors[6]\Open Then 
										If DistanceSquared(EntityX(Collider), EntityX(e\room\OBJ, True) - 3328.0 * RoomScale, EntityZ(Collider), EntityZ(e\room\OBJ, True) - 1232.0 * RoomScale) < PowTwo(5.0) Then
							...
							If e\room\NPC[8] <> Null Or e\room\NPC[9] <> Null Or e\room\NPC[10] <> Null Then
								If e\room\NPC[8]\State = 7.0 Then
									If DistanceSquared(EntityX(Collider), EntityX(e\room\OBJ, True) - 6688.0 * RoomScale, EntityZ(Collider), EntityZ(e\room\OBJ, True) - 1252.0 * RoomScale) < PowTwo(2.5) Then
							...
							e\room\NPC[5]\SoundCHN = LoopSound2(e\room\NPC[5]\Sound, e\room\NPC[5]\SoundCHN, Camera, e\room\NPC[5]\Collider, 2.0, 0.5)
							...
							Else
								...
								If e\room\NPC[3]\State3 >= 70.0 * 8.0 And e\room\NPC[3]\State3 - FPSfactor < 70.0 * 8.0 And e\room\NPC[3]\State = 7.0 Then
									If e\room\NPC[4]\Sound <> 0 Then
										FreeSound_Strict(e\room\NPC[4]\Sound) : e\room\NPC[4]\Sound = 0
									EndIf
							...
							If e\room\NPC[3]\State <> 11.0 Then
								...
								If EntityDistanceSquared(Collider, e\room\NPC[4]\Collider) > PowTwo(1.5) And EntityDistanceSquared(e\room\NPC[3]\Collider, Collider) < EntityDistanceSquared(e\room\NPC[3]\Collider, e\room\NPC[4]\Collider) Then
									e\room\NPC[4]\State = 3.0
								Else
							...
							If DistanceSquared(EntityX(e\room\NPC[3]\Collider), EntityX(e\room\RoomDoors[2]\FrameOBJ, True), EntityZ(e\room\NPC[3]\Collider), EntityZ(e\room\RoomDoors[2]\FrameOBJ, True)) < PowTwo(4.5) And DistanceSquared(EntityX(Collider), EntityX(e\room\RoomDoors[2]\FrameOBJ, True), EntityZ(Collider), EntityZ(e\room\RoomDoors[2]\FrameOBJ, True)) < PowTwo(5.0) Then
						...
						Else
							...
							e\room\NPC[3]\State = 9.0
							If DistanceSquared(EntityX(Collider), EntityX(e\room\OBJ), EntityZ(Collider), EntityZ(e\room\OBJ)) < PowTwo(4.0) Then
								e\room\RoomDoors[2]\Locked = False
								UseDoor(e\room\RoomDoors[2], False)
								e\room\RoomDoors[2]\Locked = True
								e\EventState3 = 0.0
						...
						If e\room\NPC[7] <> Null Then
							...
							If e\room\NPC[6]\State = 1.0 And e\room\NPC[7]\Sound <> 0 Then 
								If e\room\NPC[7]\SoundCHN <> 0 Then
									If ChannelPlaying(e\room\NPC[7]\SoundCHN) = False Then FreeSound_Strict(e\room\NPC[7]\Sound) : e\room\NPC[7]\Sound = 0	
								EndIf
					...
					Else
						...
						If IntroSFX(18) <> 0 Then e\SoundCHN2 = LoopSound2(IntroSFX(18), e\SoundCHN2, Camera, e\room\Objects[4], 6.0)
						...
						If e\EventState = 0.0 Then
							If PlayerRoom = e\room Then
								...
								e\room\NPC[3] = CreateNPC(NPCtypeGuard, e\room\x - 4096.0 * RoomScale + Rnd(-0.3, 0.3), e\room\y + 0.3, e\room\z + Rnd(860.0, 896.0) * RoomScale)
								...
								e\room\NPC[6] = CreateNPC(NPCtypeD, e\room\x - 3712.0 * RoomScale, e\room\y - 0.3, e\room\z - 2208.0 * RoomScale)
								ChangeNPCTextureID(e\room\NPC[6], 3)
								
								e\room\NPC[7] = CreateNPC(NPCtypeD, e\room\x - 3712.0 * RoomScale, e\room\y - 0.3, e\room\z - 2208.0 * RoomScale)
								...
								PositionEntity(Collider, e\room\x - (3072.0 + 1024.0) * RoomScale, e\room\y + 0.3, e\room\z + 192.0 * RoomScale)
								ResetEntity(Collider)
						...
						ElseIf e\EventState < 10000.0
							...
							ElseIf e\EventState > 230.0
								temp = True
								For i = 1 To 2
									If DistanceSquared(EntityX(e\room\NPC[i]\Collider), EntityX(e\room\Objects[i + 2], True), EntityZ(e\room\NPC[i]\Collider), EntityZ(e\room\Objects[i + 2], True)) > PowTwo(0.3) Then
						...
						ElseIf e\EventState < 14000.0 ;Player is inside the room
							...
							ElseIf e\EventState >= 10440.0 And e\EventState - FPSfactor < 10440.0
								UseDoor(e\room\RoomDoors[1], False)
								e\SoundCHN = PlaySound_Strict(IntroSFX(7))
						...
						ElseIf e\EventState < 20000.0
							...
							If e\EventState < 14100.0 Then ;Lights go out and SCP-173 kills the first Class D
								If e\EventState < 14060.0 Then
									...
									If BlinkTimer = -10.0 Then
										...
										RotateEntity(Curr173\Collider, 0.0, EntityYaw(Curr173\Collider), 0.0)
								...
								ElseIf e\EventState < 14065.0
									...
									SetNPCFrame(e\room\NPC[1], 678.0)
								...
								If e\EventState > 14080.0 And e\EventState - FPSfactor < 14080.0 Then PlaySound_Strict(IntroSFX(12))
								CameraShake = 3.0
							ElseIf e\EventState < 14200.0 ;Kills the other class d
								Animate2(e\room\NPC[1]\OBJ, AnimTime(e\room\NPC[1]\OBJ), 678.0, 711.0, 0.5, False)
								...
								If e\EventState < 14130.0 Then 
									...
									BlinkTimer = -10.0 : LightBlink = 1.0
								Else 
									Animate2(e\room\NPC[2]\obj, AnimTime(e\room\NPC[2]\OBJ), 712.0, 779.0, 0.5, False)
									Curr173\Idle = False
								EndIf
								If e\EventState > 14100 And e\EventState - FPSfactor < 14100.0 Then PlaySound_Strict(IntroSFX(8))
						...
						ElseIf e\EventState < 30000.0
							...
							If e\EventState < 20100.0 Then
								CameraShake = 2.0
							Else
								If e\EventState < 20200.0 Then ;Lights go out again and 173 teleports next to the guard
									...
									BlinkTimer = -10.0 : LightBlink = 1.0
									CameraShake = 3.0
								ElseIf e\EventState < 20300.0 ;Lights on, the guard starts shooting at SCP-173
									...
									If e\EventState > 20260.0 And e\EventState - FPSfactor < 20260.0 Then PlaySound_Strict(IntroSFX(12))
				...
				If PlayerRoom = e\room Then
					...
					TempLightVolume = 4.0			
				Else
					DebugLog("Intro scene finished successfully!")
					RemoveEvent(e)		
				EndIf	
				;[End Block]
			Case "buttghost"
				...
				If PlayerRoom = e\room Then
					...
					If CoffinDistance < 1.5 Then 
						...
						If (Not Contained106) And e\EventName="coffin106" And e\EventState2 = 0 Then
							CreateConsoleMsg("THIS SHOULD HAPPEN")
							de.Decals = CreateDecal(0, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True) -1531.0*RoomScale, EntityZ(e\room\Objects[1],True), 90, Rand(360), 0)
							de\Size = 0.05 : de\SizeChange = 0.001 : EntityAlpha(de\obj, 0.8) : UpdateDecals()
							
							If Curr106\State > 0 Then
								PositionEntity Curr106\Collider, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True) -10240*RoomScale, EntityZ(e\room\Objects[1],True)
								Curr106\State = -0.1
					...
					If WearingNightVision <> 0 Then
						...
						If (CoffinDistance < 4.0) And (hasBatteryFor895) And (Not Wearing714) Then
							TurnEntity Collider, 0,angleDist(point_direction(EntityX(Collider,True),EntityZ(Collider,True),EntityX(e\room\Objects[1],True),EntityZ(e\room\Objects[1],True))+90+Sin(WrapAngle(e\EventState3/10)),EntityYaw(Collider))/4,0,True
					...
					If e\EventState = 0 Then
						If e\room\dist < 8 And e\room\dist > 0 Then
							...
							Else
								...
								e\room\NPC[0]=CreateNPC(NPCtypeD, EntityX(e\room\RoomDoors[0]\obj,True), EntityY(e\room\RoomDoors[0]\obj,True)+0.5, EntityZ(e\room\RoomDoors[0]\obj,True))
			...
			Case "gateaentrance"
				...
				If PlayerRoom = e\room Then 
					...
					Else
						...
						If EntityDistanceSquared(Collider, e\room\Objects[1])<PowTwo(4.0) Then
					...
					If e\EventState2 = 1 Then ;in the second room
								...
								For i = 9 To 10
									...
									If dist<PowTwo(6.0) Then 
										If dist<PowTwo(100.0*RoomScale) Then
											...
											If KillTimer = 0 Then
												DeathMSG = GetLocalString("Deaths", "106crush")
						...
						If EntityY(Collider) < -1600*RoomScale Then
							...
							Else ;the player is not at the exit, must've fallen down
								...
								If KillTimer => 0 Then 
									PlaySound_Strict HorrorSFX(8)
									DeathMSG = GetLocalString("Deaths", "106fall")
							...
							If EntityInView(e\room\Objects[0], Camera) Then
								...
								If MouseHit1 Then
									...
									If e\EventState2 < 2 Then
										If SelectedItem<>Null Then
											If SelectedItem\itemtemplate\tempname="quarter" Lor SelectedItem\itemtemplate\tempname="coin" Then
									...
									ElseIf e\EventState2 = 1 And (Not inserted) Then
										Using294=False
										Msg = GetLocalString("Messages", "2491")
										MsgTimer = 70*5
									ElseIf (Not inserted) Then
										Using294=False
										Msg = GetLocalString("Messages", "2492")
										MsgTimer = 70*5
									EndIf
								EndIf
							EndIf
			...
			Case "room2closets"
				...
				Else
					...
					If e\EventState < 70.0 * 3.0 Then
						...
						e\room\NPC[0]\State = 1.0
						If e\EventState > 70.0 * 2.9 And e\EventState - FPSfactor =< 70.0 * 2.9 Then PlaySound2(IntroSFX(15), Camera, e\room\OBJ, 15.0)
					...
					Else
						...
						If e\EventState > 70.0 * 9.0 And e\EventState - FPSfactor =< 70.0 * 9.0 Then
							it.Items = CreateItem("wallet", EntityX(e\room\Objects[2], True), EntityY(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True))
							...
							For i = 0 To 1
								it2.Items = CreateItem("quarter", 1.0, 1.0, 1.0)
								it2\Picked = True
								it2\Dropped = -1
			...
			Case "room2elevator"
				...
				If e\EventState = 0 Then
					If e\room\dist < 8.0 And e\room\dist > 0 Then
						e\room\NPC[0]=CreateNPC(NPCtypeGuard, EntityX(e\room\obj,True), EntityY(e\room\obj,True)+0.5, EntityZ(e\room\obj,True))
			...
			Case "room2tesla"
				...
				If temp And EntityY(Collider, True) > EntityY(e\room\obj,True) And EntityY(Collider, True) < 4.0 Then
					...
					If e\EventState = 0 Then
						If e\room\dist < 8 Then
							...
							If temp2
								If e\EventStr = "" And PlayerRoom = e\room
									...
									e\room\NPC[0] = CreateNPC(NPCtypeClerk,EntityX(e\room\Objects[temp3],True),EntityY(e\room\Objects[temp3],True)+0.5,EntityZ(e\room\Objects[temp3],True))
						...
						If Curr106\State < -10 And e\EventState = 0 Then 
							For i = 0 To 2
								If DistanceSquared(EntityX(Curr106\Collider),EntityX(e\room\Objects[i],True),EntityZ(Curr106\Collider),EntityZ(e\room\Objects[i],True)) < PowTwo(300.0*RoomScale) And EntityY(Curr106\Collider) - EntityY(e\room\Objects[i],True) < 2 Then
									;play the activation sound
									If KillTimer => 0 Then 
										StopChannel(e\SoundCHN)
										e\SoundCHN = PlaySound2(TeslaActivateSFX, Camera, e\room\Objects[3],4.0,0.5)
					...
					Else
						...
						Else
							...
							If e\EventState < 70 Then 
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If DistanceSquared(EntityX(Collider),EntityX(e\room\Objects[i],True),EntityZ(Collider),EntityZ(e\room\Objects[i],True)) < PowTwo(250.0*RoomScale) And EntityY(Collider) - EntityY(e\room\Objects[i],True) < 2 Then
											ShowEntity Light
											LightFlash = 0.4
											CameraShake = 1.0
											Kill()
											DeathMSG = GetLocalString("Deaths", "tesla")
										EndIf
									Next
								EndIf
								...
								If Curr106\State < -10 Then
									For i = 0 To 2
										If DistanceSquared(EntityX(Curr106\Collider),EntityX(e\room\Objects[i],True),EntityZ(Curr106\Collider),EntityZ(e\room\Objects[i],True)) < PowTwo(250.0*RoomScale) And EntityY(Curr106\Collider) - EntityY(e\room\Objects[i],True) < 2 Then
											ShowEntity Light
											LightFlash = 0.3
											If ParticleAmount > 0
				...
				If e\room\NPC[0] <> Null
					If e\EventStr = "step1" And e\room\NPC[0]\State <> 3
						If e\EventState = 0
							For i = 0 To 2
								If DistanceSquared(EntityX(e\room\NPC[0]\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\NPC[0]\Collider),EntityZ(e\room\Objects[i],True)) < PowTwo(400.0*RoomScale) And EntityY(e\room\NPC[0]\Collider) - EntityY(e\room\Objects[i],True) < 2 Then
									StopChannel(e\SoundCHN)
									e\SoundCHN = PlaySound2(TeslaActivateSFX, Camera, e\room\Objects[3],4.0,0.5)
									HideEntity e\room\Objects[4]
									e\EventState = 1
			...
			Case "room2trick"
				...
				If PlayerRoom = e\room Then
					If EntityDistanceSquared(e\room\obj,Collider)<PowTwo(2.0) Then
						If EntityDistanceSquared(Collider, Curr173\obj)<36.0 Lor EntityDistanceSquared(Collider, Curr106\obj)<PowTwo(6.0) Then
							RemoveEvent(e)
						Else
							DebugLog "%@@= \ {2E6C2=FD gi`h]c"
								...
								If e\room\grid\grid[ix+(iy*gridsz)]>0 Then
									...
									Select e\room\grid\grid[ix+(iy*gridsz)]
										...
										Case 7
											...
											it = CreateItem("nvg",e\room\x+ix*2.0-(Sin(EntityYaw(tempInt,True))*504.0*RoomScale)+(Cos(EntityYaw(tempInt,True))*16.0*RoomScale), 8.0+(80.0*RoomScale), e\room\z+iy*2.0+(Cos(EntityYaw(tempInt,True))*504.0*RoomScale)+(Sin(EntityYaw(tempInt,True))*16.0*RoomScale))
											EntityType (it\collider, HIT_ITEM)
									End Select
									
									If e\room\grid\grid[ix+(iy*gridsz)]=6 Lor e\room\grid\grid[ix+(iy*gridsz)]=5 Then
										dr=CreateDoor(e\room\zone,e\room\x+(ix*2.0)+(Cos(EntityYaw(tempInt,True))*240.0*RoomScale),e\room\y+8.0,e\room\z+(iy*2.0)+(Sin(EntityYaw(tempInt,True))*240.0*RoomScale),EntityYaw(tempInt,True)+90.0,Null,False,3,False,"")
										PositionEntity(dr\Buttons[0], EntityX(dr\Buttons[0], True) + (Cos(EntityYaw(tempInt, True)) * 0.05), EntityY(dr\Buttons[0], True), EntityZ(dr\Buttons[0], True) + (Sin(EntityYaw(tempInt, True)) * 0.031), True)
										PositionEntity(dr\Buttons[1], EntityX(dr\Buttons[1], True) + (Cos(EntityYaw(tempInt, True)) * 0.05), EntityY(dr\Buttons[1], True), EntityZ(dr\Buttons[1], True) + (Sin(EntityYaw(tempInt, True)) * 0.05), True)
					...
					;	HideEntity Meshes[6]
						...
						For it.Items = Each Items
							If (EntityY(it\collider,True)>=8.0) And (EntityY(it\collider,True)<=12.0) Then
								DebugLog it\localname+" is within Y limits"
			...
			Case "room3pit1048"
				...
				If PlayerRoom = e\room Then
					If e\room\Objects[2] = 0 Then
						...
						SetAnimTime(e\room\Objects[2], 487)
						...
						For i = 1 To CountSurfaces(e\room\Objects[2])
							...
							CreateConsoleMsg "texname: "+texname
							If Lower(texname) = "1048_1.jpg" Then
								PaintSurface sf, brush
							EndIf
							;If texname<>"" Then FreeTexture t ;MAV????
							FreeBrush b
						Next
...
;								If EntityDistanceSquared(Curr096\Collider, e\room\Objects[8])<PowTwo(5.76) Then
;									Curr096\State2 = 0
;								Else
;									Curr096\State2 = 1001
;								EndIf	
...
;						EndIf
			...
			Case "room2storage"
					...
					If Abs(EntityX(Collider)-e\room\x)<8.0 Then
						If Abs(EntityZ(Collider)-e\room\z)<8.0 Then
							...
							If e\EventState < 30 Then
								e\EventState3 = e\EventState3-FPSfactor*0.01*(e\EventState3/(0.4)-1)
								temp = 0
				...
				If PlayerRoom = e\room Then
					...
					If temp Then
						e\EventState3 = e\EventState3-FPSfactor*0.01*(e\EventState3/(0.5)-1)
					EndIf
					
					LightVolume = TempLightVolume*e\EventState3
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 50
								e\room\NPC[1]=CreateNPC(NPCtypeGuard, EntityX(e\room\obj)+Cos(e\room\angle+90)*600*RoomScale, EntityY(e\room\obj)+0.35, EntityZ(e\room\obj)+Sin(e\room\angle+90)*600*RoomScale)
							...
							If (it\Dropped=1 And Abs(TFormedX())<264) Lor it\Dropped=-1 Then
								...
								If it\Dropped=1 Then
									For i = - 1 To 1 Step 2
										...
										it2.items = CreateItem(it\itemtemplate\tempname, TFormedX(), EntityY(it\collider), TFormedZ(), it\itemtemplate\namespec)
										RotateEntity(it2\collider, EntityPitch(it\collider),EntityYaw(it\collider),0)
										EntityType(it2\collider, HIT_ITEM)
									Next
								Else
									For it2.items = Each Items
										If it2<>it And it2\distsquared < PowTwo(15.0) Then
			...
			Case "room3door"
				...
				If PlayerRoom = e\room Then
					If EntityDistanceSquared(e\room\obj,Collider)<PowTwo(2.5) Then ;2.5
						For do.doors = Each Doors
							If Abs(EntityX(do\obj,True)-EntityX(Collider))<2.0 Then
								If Abs(EntityZ(do\obj,True)-EntityZ(Collider))<2.0 Then
									If (Not EntityInView(do\obj,Camera)) Then
										If do\open Then
											DebugLog "%@@= \ {2E6C2=FD gi`h]c" ;Thank you for this, really helpful!
											do\open = False
											do\openstate = 0
											BlurTimer = 100
											CameraShake = 3.0											
			...
			Case "room4"
				...
				If e\EventState < MilliSecs() Then
					If PlayerRoom <> e\room Then
						If DistanceSquared(EntityX(Collider),EntityX(e\room\obj),EntityZ(Collider),EntityZ(e\room\obj))<PowTwo(16) Then
							For n.NPCs = Each NPCs
								If n\NPCtype = NPCtype049 Then
									If n\State = 2 And EntityDistanceSquared(Collider,n\Collider)>PowTwo(16) Then
							...
							If EntityVisible(e\room\Objects[2],Camera) Then 							
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>13*70 And e\EventState3-FPSfactor=<13*70
										Msg = GetLocalString("Messages", "0121")
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>31*70 And e\EventState3-FPSfactor=<31*70
										...
										Msg = GetLocalString("Messages", "0122")
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>49*70 And e\EventState3-FPSfactor=<49*70
										Msg = GetLocalString("Messages", "0123")
										MsgTimer = 8*70
										Injuries=Injuries+0.3
									...
									ElseIf e\EventState3>74*70 And e\EventState3-FPSfactor=<74*70
										...
										Msg = GetLocalString("Messages", "0124")
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
										DeathMSG = GetLocalString("Deaths", "012")
										Kill()
									EndIf
			...
			Case "room035"
				...
				If PlayerRoom = e\room Then
					...
					If e\EventState = 0 Then
						If EntityDistanceSquared(Collider, e\room\Objects[3])<PowTwo(2) Then
							n.NPCs = CreateNPC(NPCtypeD, EntityX(e\room\Objects[4],True),EntityY(e\room\Objects[4],True)+0.5,EntityZ(e\room\Objects[4],True))
					...
					ElseIf e\EventState > 0
						...
						If e\EventState=1 Then
							If EntityDistanceSquared(Collider, e\room\Objects[3])<PowTwo(1.2) Then
								If EntityInView(e\room\NPC[0]\obj, Camera) Then
							...
							If UpdateLever(e\room\Levers[0],(e\EventState2=20)) = 0 Then
								...
								Else ;gas valves closed
									...
									If e\room\NPC[0]\State = 0 Then
										...
										If Rand(500)=1 Then
											If EntityDistanceSquared(e\room\NPC[0]\Collider, e\room\Objects[4])>PowTwo(2.0) Then
									...
									ElseIf e\room\NPC[0]\State = 1
										If e\room\NPC[0]\State2 = 1 Then
											...
											If EntityDistanceSquared(e\room\NPC[0]\Collider, e\room\Objects[4])<PowTwo(0.2) Then e\room\NPC[0]\State = 0
										Else
											RotateEntity e\room\NPC[0]\obj, 0, e\room\angle-180, 0, True
											If EntityDistanceSquared(e\room\NPC[0]\Collider, e\room\Objects[4])>PowTwo(2.0) Then e\room\NPC[0]\State = 0
										EndIf
				...
				Else	
					If e\EventState=0 Then	
						If e\Sound = 0 Then
							If EntityDistanceSquared(Collider, e\room\obj) < PowTwo(20.0) Then
			...
			Case "room049"
				...
				If PlayerRoom = e\room Then
					...
					Else
						...
						If e\EventState = 0 Then
							If e\EventStr = "" And QuickLoad_CurrEvent = Null
								QuickLoad_CurrEvent = e
								e\EventStr = "load0"
							EndIf
							...
							If EntityDistanceSquared(e\room\Objects[11],Collider)<EntityDistanceSquared(e\room\Objects[12],Collider) Then
								it = CreateItem("paper", EntityX(e\room\Objects[11],True),EntityY(e\room\Objects[11],True),EntityZ(e\room\Objects[11],True), "sector02")
								EntityType it\collider,HIT_ITEM
							Else
								it = CreateItem("paper", EntityX(e\room\Objects[12],True),EntityY(e\room\Objects[12],True),EntityZ(e\room\Objects[12],True), "sector02")
								EntityType it\collider,HIT_ITEM
							EndIf
						ElseIf e\EventState > 0
							...
							If temp And x Then
								...
								If e\room\NPC[0]\Idle > 0
									i = 0
									If EntityDistanceSquared(Collider,e\room\RoomDoors[1]\frameobj)<PowTwo(3) Then
										i = 1
									ElseIf EntityDistanceSquared(Collider,e\room\RoomDoors[3]\frameobj)<PowTwo(3) Then
										i = 3
									EndIf
									If i > 0
...
;						EndIf
				...
				If PlayerRoom = e\room Then
					...
					ElseIf e\EventState < 10000
						...
						If e\EventState = 1 Then 
							e\EventState = 2
						ElseIf e\EventState = 2
							If EntityDistanceSquared(e\room\Objects[0], Collider)<PowTwo(3.0) Then
				...
				If PlayerRoom = e\room Then
					...
					Else
						...
						If (e\EventState<65) Then
							If e\EventState>10 And (Rand(200/FPSfactor)=1) Then
						...
						Select e\EventState
							...
							Case 9
								...
								Animate2(e\room\Objects[5], AnimTime(e\room\Objects[5]), 2, 122, 0.2)
							...
							Case 66
								Animate2(e\room\Objects[3], AnimTime(e\room\Objects[3]), 492, 534, 0.25, False)
								Animate2(e\room\Objects[4], AnimTime(e\room\Objects[4]), 434, 466, 0.25, False)
								Animate2(e\room\Objects[5], AnimTime(e\room\Objects[5]), 434, 494, 0.25, False)
								...
								If AnimTime(e\room\Objects[3])>533 Then
									...
									FreeEntity e\room\Lights[0]
									FreeEntity e\room\LightSprites[0]
									FreeEntity e\room\LightSprites2[0]
									e\room\Lights[0] = 0
									e\room\LightSprites[0] = 0
									e\room\LightSprites2[0] = 0
									
									FreeEntity e\room\Lights[2]
									FreeEntity e\room\LightSprites[2]
									FreeEntity e\room\LightSprites2[2]
									e\room\Lights[2] = 0
									e\room\LightSprites[2] = 0
									e\room\LightSprites2[2] = 0
									
									FreeEntity e\room\Objects[3]
									FreeEntity e\room\Objects[4]
									FreeEntity e\room\Objects[5]
									FreeEntity e\room\Objects[6]
								EndIf
							Case 67
								If (Rand(100/FPSfactor)=1) Then
									DeathMSG = GetLocalString("Deaths", "205")
								...
								If CheckTriggers() <> "205scene_enter" Then
									e\EventState = 68
									e\EventState2 = -Abs(e\EventState2)
								EndIf
							Case 68
								e\EventState2 = e\EventState2 + FPSfactor
							...
							Default
								e\EventState3 = e\EventState3 + FPSfactor
								If e\EventState3 > 50 Lor Rand(3) > 1 Then
								...
								Else
									HideEntity e\room\Objects[1]
									HideEntity e\room\LightSprites[2]
									HideEntity e\room\LightSprites2[2]
									HideEntity e\room\LightSprites[0]
									HideEntity e\room\LightSprites2[0]
								EndIf
						End Select
					EndIf
			...
			Case "room860"
				...
				If PlayerRoom = e\room And fr<>Null Then 
					...
					If e\EventState=1.0 Then ;the player is in the forest
						...
						If e\room\NPC[0]<>Null
							If e\room\NPC[0]\State = 0 Lor EntityDistanceSquared(Collider, e\room\NPC[0]\Collider)>PowTwo(20) Then
						...
						For i = 0 To 1
							If EntityDistanceSquared(fr\Door[i], Collider)<PowTwo(0.5) Then ;0.5
								If EntityInView(fr\Door[i], Camera) Then
									...
									If MouseHit1 Then
										...
										Else
											...
											Msg = GetLocalString("Messages", "860door")
											MsgTimer = 70*5
										EndIf
									EndIf
								EndIf
					...
					Else
						...
						If EntityYaw(e\room\Objects[3])=0.0 Then
							...
							If (DistanceSquared(EntityX(e\room\Objects[3],True),EntityX(Collider,True),EntityZ(e\room\Objects[3],True),EntityZ(Collider,True))<PowTwo(1.0)) Then
								DrawHandIcon = True
			...
			Case "testroom"
				...
				If PlayerRoom = e\room Then
					If e\EventState = 0 Then
						...
						e\EventState = 1
					Else
						If EntityDistanceSquared(Collider, e\room\Objects[6]) < PowTwo(2.5) Then
				...
				If PlayerRoom = e\room	
					If Curr173\Idle = 0 Then 
						...
						Else
							...
							ElseIf dist > PowTwo(1.4) Then
								;if the player moves a bit further and blinks, 173 attacks
								If e\EventState > 70*12 And BlinkTimer =< -10 Lor (Not EntityInView(Curr173\obj, Camera)) Then
									If EntityDistanceSquared(Curr173\Collider, e\room\Objects[0]) > PowTwo(5) Then
				...
				If PlayerRoom = e\room Then	
					...
					ElseIf e\EventState = 1
						...
						If dist<PowTwo(2.0) Then 
							...
							If e\EventState2=0 Then
								ShowEntity e\room\Objects[2]
								If EntityDistanceSquared(Curr173\Collider,e\room\Objects[4]) < PowTwo(3.0) Then
									If (BlinkTimer<-10 Lor (Not EntityInView(Curr173\obj,Camera))) And Curr173\Idle = 0 Then
										...
										If WearingHazmat < 1 Then
											...
											Msg = GetLocalString("Messages", "008173breakwindow")
											MsgTimer = 70*8
										EndIf
							...
							If WearingHazmat < 1 And Bloodloss>0.0
								If Infect=0
									Infect=1
				...
				If (Not Contained106) Then
					...
					If e\EventState > 0 Then 
						...
						If e\EventState>200 Then
							If e\room\NPC[0]=Null Then
								e\room\NPC[0]=CreateNPC(NPCtypeD, EntityX(e\room\obj), EntityY(e\room\obj)+900.0*RoomScale, EntityZ(e\room\obj))
					...
					Select e\EventState
						...
						Case 2
							...
							If (prevFrame>646.0) Then
								If (PlayerRoom = e\room) Then
									...
									Msg = GetLocalString("Messages", "1048a")
									MsgTimer = 70.0 * 3.0
								Else
									e\EventState3 = 70*30
								EndIf
						...
						Case 3
							...
							If (e\EventState2>70*15) Then
								DeathMSG = GetLocalString("Deaths", "1048a")
								Kill()
								e\EventState = 4
								RemoveEvent(e)
							EndIf
			...
			Case "room1162"
				...
				If PlayerRoom = e\room
					...
					For it.Items = Each Items
						If (it\Picked = 0)
							If EntityDistanceSquared(it\collider,e\room\Objects[0]) < PowTwo(0.75) Then
								Pick1162% = False
							EndIf
						EndIf
					Next
					
					If Pick1162 And EntityDistanceSquared(e\room\Objects[0],Collider) < PowTwo(0.75) Then
						DrawHandIcon = True
						If MouseHit1 Then GrabbedEntity = e\room\Objects[0]
					EndIf
						...
						Else
							...
							For i = 0 To MaxItemAmount-1
								...
								If Rand(8)=1 Then
									...
									Select (e\EventState)
										Case 1
											itemName = GetLocalString("Items", "lkey")
										Case 2
											itemName = GetLocalString("Items", "hearing")
										Case 3
											itemName = GetLocalString("Items", "coin")
										Case 4
											itemName = GetLocalString("Items", "ticket")
										Case 5
											itemName = GetLocalString("Items", "970ds")
										Case 6
											itemName = GetLocalString("Items", "oldbadge")
									End Select
									
									Local itemExists% = False
									For it.Items = Each Items
										If (it\localname = itemName) Then
											itemExists = True
											e\EventState3 = 1.0
											e\EventState = 0.0
											Exit
					...
					If e\EventState3 = 1.0
						...
						For itt.ItemTemplates = Each ItemTemplates
								...
								Select Inventory(e\EventState2)\itemtemplate\tempname
									...
									Case "paper"
										If (itt\tempname = "paper") And Rand(12)=1 Then
											shouldCreateItem = True
										EndIf
									Case "badgasmask","gasmask","heavygasmask","supergasmask","hazmat0","hazmat","hazmat2","hazmat3"
										If (itt\tempname = "badgasmask" Lor itt\tempname = "gasmask" Lor itt\tempname = "heavygasmask" Lor itt\tempname = "supergasmask" Lor itt\tempname = "hazmat0" Lor itt\tempname = "hazmat" Lor itt\tempname = "hazmat2" Lor itt\tempname = "hazmat3") And Rand(2)=1
											shouldCreateItem = True
										EndIf
									Case "key1","key2","key3"
										If (itt\tempname = "key1" Lor itt\tempname = "key2" Lor itt\tempname = "key3" Lor itt\tempname = "misc") And Rand(6)=1
											shouldCreateItem = True
										EndIf
									Case "badvets","vest","finevest"
										If (itt\tempname = "badvest" Lor itt\tempname = "vest" Lor itt\tempname = "finevest") And Rand(2)=1
											shouldCreateItem = True
										EndIf
									Default
							...
							If (shouldCreateItem) Then
								...
								it=CreateItem(itt\tempname,EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True), itt\namespec)
								EntityType(it\collider, HIT_ITEM)
					...
					ElseIf e\EventState3 = 2.0
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6)=1
								...
								e\EventState3 = 0.0
								If Injuries > 15
									DeathMSG = GetLocalString("Deaths", "1162")
								...
								Else
									PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
									LightFlash = 5.0
									Msg = GetLocalString("Messages", "1162oof")
									MsgTimer = 70*5
								EndIf
								Exit
							EndIf
					...
					ElseIf e\EventState3 >= 3.0
						...
						Else
							...
							Else
								PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
								LightFlash = 5.0
								Msg = GetLocalString("Messages", "1162oof2")
								MsgTimer = 70*5
							EndIf
							e\EventState2 = 0.0
						EndIf
						Select e\EventState
							Case 1
								it = CreateItem("key",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True), "lkey")
							Case 2
								it = CreateItem("paper",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True), "hearing")
							Case 3
							...
							Case 5
								it = CreateItem("paper",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True), "970ds")
								it\state = 1
							Case 6
								it = CreateItem("badge",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True), "oldbadge")
						End Select
						EntityType(it\collider, HIT_ITEM)
						GiveAchievement(Achv1162)
						MouseHit1 = False
			...
			Case "room_gw"
				...
				If PlayerRoom = e\room
					If e\EventState = 0.0
						If e\EventState3 = 0.0 And EntityDistanceSquared(e\room\Objects[0],Collider) < PowTwo(1.4) Then
							e\EventState = 1.0
							If brokendoor
								If e\Sound2 <> 0 Then FreeSound_Strict(e\Sound2) : e\Sound2 = 0
						...
						ElseIf EntityDistanceSquared(e\room\Objects[0],Collider) > PowTwo(2.4) Then
							e\EventState3 = 0.0
						EndIf
					Else
				...
				If e\EventState = 1
					If e\EventState2 < 0
						If e\EventState2 = -(70*5)
							For sc.SecurityCams = Each SecurityCams
								If sc\room = e\room
									If EntityDistanceSquared(sc\ScrObj,Camera) < PowTwo(5.0) Then
					...
					ElseIf e\EventState2 = 0
						If e\room\NPC[0] <> Null
							...
							Local AdjDistSquared1# = 0.0
							Local AdjDistSquared2# = 0.0
							Local Adj1% = -1
							Local Adj2% = -1
							For i = 0 To 3
								If e\room\AdjDoor[i]<>Null
									If Adj1 = -1
										AdjDistSquared1 = EntityDistanceSquared(e\room\Objects[7],e\room\AdjDoor[i]\frameobj)
										Adj1 = i
									Else
										AdjDistSquared2 = EntityDistanceSquared(e\room\Objects[7],e\room\AdjDoor[i]\frameobj)
										Adj2 = i
									EndIf
								EndIf
							Next
							If AdjDistSquared1 > AdjDistSquared2
					...
					ElseIf e\EventState2 = 2
						If e\room\NPC[0]\PathStatus <> 1
							...
							DebugLog "ccccccccc"
						Else
							If EntityDistanceSquared(e\room\NPC[0]\Collider,e\room\RoomDoors[0]\frameobj) < PowTwo(5.0) Then
								e\room\RoomDoors[0]\locked = True
								e\room\RoomDoors[1]\locked = True
			...
			Case "096spawn"
				...
				If e\room\dist < HideDistance
					...
					If e\EventState <> 2
						If Curr096<>Null
							If EntityDistanceSquared(Curr096\Collider,Collider) < PowTwo(40.0) Then
								e\EventState = 2
			...
			Case "room2offices035"
				...
				If is035released
					If e\room\dist < 8
						If e\room\NPC[0]=Null
							e\room\NPC[0]=CreateNPC(NPCtypeD,e\room\x,e\room\y+0.5,e\room\z)
						...
						If e\room\NPC[1]=Null
							If EntityDistanceSquared(e\room\NPC[0]\Collider,Collider) < PowTwo(2.5)
								e\room\NPC[1]=CreateNPC(NPCtypeTentacle,EntityX(e\room\NPC[0]\Collider),EntityX(e\room\NPC[0]\Collider),EntityZ(e\room\NPC[0]\Collider))
			...
			If PlayerRoom = e\room Then
				If e\EventState < 2.0
					...
					For i = 0 To 14
						...
						n\Angle = Rnd(360)
						n\State2 = 0
						If EntityDistanceSquared(n\Collider,Collider)< PowTwo(10.0) Then
							n\State = 2
						EndIf
					Next
				EndIf
				...
				If QuickLoad_CurrEvent = Null
					...
					If EntityDistanceSquared(Collider,e\room\obj) > PowTwo(40.0)
						For du.Dummy1499 = Each Dummy1499
							HideEntity du\obj
						Next
					Else
				...
				If PlayerRoom = e\room Then
					...
					If EntityY(Collider)>1040.0*RoomScale Then
						...
						If e\EventState = 0 Then
							...
							e\room\NPC[0] = CreateNPC(NPCtypeApache, e\room\x, e\room\y+100.0, e\room\z)
							e\room\NPC[0]\State = 1
						...
						Else
							...
							If e\EventState < 2.0 And SelectedEnding = "" Then 
								...
								If EntityDistanceSquared(Collider, e\room\Objects[10]) < PowTwo(320*RoomScale) Then
									...
									CreateNPC(NPCType682, EntityX(e\room\NPC[3]\Collider), 12192.0*RoomScale, EntityZ(e\room\NPC[3]\Collider))
								EndIf								
							Else
								ShouldPlay = 6
								e\EventState=e\EventState+FPSfactor
								...
								Else
									...
									Else
										If SelectedEnding = "B3" Then
												...
												For n.NPCs = Each NPCs
													If n\NPCtype = NPCtypeMTF
														If n\State = 5
															...
															n\LastSeen = 70*300
														EndIf
														If EntityDistanceSquared(n\Collider,Collider) < PowTwo(3.0)
															n\State = 5
															n\PathStatus = 0
															n\PathTimer = 0
															n\CurrSpeed = 0
											...
											For n.NPCs = Each NPCs
												If n\NPCtype = NPCtypeMTF
													If n\State = 5 And EntityDistanceSquared(n\Collider,Collider) < PowTwo(3.0) Then
														If e\EventState3 = 0.0
						...
						;EntityAlpha Fog, 1.0-Max(Min(EntityDistance(Collider, e\room\objects[3])/10000.0,1.0),0.0)
						HideEntity Fog
						...
						If ParticleAmount > 0 And Rand(3) = 1 Then
							...
							If Contained106=False Then
								...
								If e\EventState3 = 0.0 Then 
									If Abs(EntityY(Collider)-EntityY(e\room\Objects[11],True))<1.0 Then
										If DistanceSquared(EntityX(Collider),EntityX(e\room\Objects[11],True),EntityZ(Collider),EntityZ(e\room\Objects[11],True)) < PowTwo(12.0) Then
											Curr106\State = 100000
											HideEntity Curr106\obj
								...
								ElseIf e\EventState3 = 1.0
									...
									For i = 5 To 8
										If EntityDistanceSquared(e\room\NPC[i]\Collider,Collider) > PowTwo(4) Then
											e\room\NPC[i]\State = 3
										Else
											e\room\NPC[i]\State = 1
										EndIf
									...
									If Abs(EntityY(Collider)-EntityY(e\room\Objects[11],True))<1.0 Then
										If DistanceSquared(EntityX(Collider),EntityX(e\room\Objects[11],True),EntityZ(Collider),EntityZ(e\room\Objects[11],True)) < PowTwo(7.0) Then
							...
							Else ;contained106 = true
								...
								Else
									...
									For i = 5 To 8
										...
										Else
											If EntityDistanceSquared(e\room\NPC[i]\Collider,Collider)<PowTwo(6.0)
												e\room\NPC[i]\State = 5
